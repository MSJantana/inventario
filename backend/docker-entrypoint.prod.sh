...
case "$DATABASE_URL" in
  ""|*'${'*)
    ...
    export DATABASE_URL="$(node -p 'const user = process.env.DB_USER_VALUE || ""; const password = process.env.DB_PASSWORD_VALUE || ""; const database = process.env.DB_NAME_VALUE || ""; const host = process.env.DB_HOST_VALUE || "db"; const port = process.env.DB_PORT_VALUE || "3306"; const encode = encodeURIComponent; "mysql://" + encode(user) + ":" + encode(password) + "@" + host + ":" + port + "/" + encode(database)')"
